...
for j in range(len(folders)):
    ...
    images=glob.glob("./data/"+folders[j]+"/images/*")
    
    for i in range(len(images)):
        image = Image.open(images[i]);
        width, height = image.size;
        newheight=(640*height)/width;
        new_image = image.resize((640, int(newheight)));
    ...
    labels=glob.glob("./data/"+folders[j]+"/labels/*")
    for i in range(len(labels)):
        os.system("cp "+labels[i]+" ./newdata/"+folders[j]+"/labels/"+labels[i][14+1+len(folders[j])::])
    
    
    images=glob.glob("./newdata/"+folders[j]+"/images/*")
    for i in range(len(images)):
        if i%2==0:
            print(images[i])
    ...
    labels=glob.glob("./newdata/"+folders[j]+"/labels/*")
    for i in range(len(labels)):
        if i%2==0:
